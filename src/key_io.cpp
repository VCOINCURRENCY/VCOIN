...
public:
    ...
    std::string operator()(const libvcoin::SproutPaymentAddress& zaddr) const
    {
    ...
    std::string operator()(const libvcoin::SaplingPaymentAddress& zaddr) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zaddr;
    ...
    std::string operator()(const libvcoin::InvalidEncoding& no) const { return {}; }
};
...
public:
    ...
    std::string operator()(const libvcoin::SproutViewingKey& vk) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << vk;
...
public:
    ...
    std::string operator()(const libvcoin::SproutSpendingKey& zkey) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zkey;
    ...
    std::string operator()(const libvcoin::SaplingExtendedSpendingKey& zkey) const
    {
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
        ss << zkey;
        // ConvertBits requires unsigned char, but CDataStream uses char
...
std::string EncodePaymentAddress(const libvcoin::PaymentAddress& zaddr)
{
    return boost::apply_visitor(PaymentAddressEncoder(Params()), zaddr);
}

libvcoin::PaymentAddress DecodePaymentAddress(const std::string& str)
{
    std::vector<unsigned char> data;
    if (DecodeBase58Check(str, data)) {
        const std::vector<unsigned char>& zaddr_prefix = Params().Base58Prefix(CChainParams::ZCPAYMENT_ADDRRESS);
        if ((data.size() == libvcoin::SerializedSproutPaymentAddressSize + zaddr_prefix.size()) &&
            ...
            libvcoin::SproutPaymentAddress ret;
            ss >> ret;
            return ret;
        }
    }
    ...
    if (bech.first == Params().Bech32HRP(CChainParams::SAPLING_PAYMENT_ADDRESS) &&
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin(), bech.second.end())) {
            ...
            libvcoin::SaplingPaymentAddress ret;
            ss >> ret;
            return ret;
        }
    }
    return libvcoin::InvalidEncoding();
}
...
libvcoin::SpendingKey DecodeSpendingKey(const std::string& str)
{
    ...
    if (bech.first == Params().Bech32HRP(CChainParams::SAPLING_EXTENDED_SPEND_KEY) &&
        ...
        if (ConvertBits<5, 8, false>([&](unsigned char c) { data.push_back(c); }, bech.second.begin(), bech.second.end())) {
            CDataStream ss(data, SER_NETWORK, PROTOCOL_VERSION);
            libvcoin::SaplingExtendedSpendingKey ret;
            ss >> ret;
            memory_cleanse(data.data(), data.size());
            return ret;
        }
