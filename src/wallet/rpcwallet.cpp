...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "                             The recipient will receive less Vcoin than you enter in the amount field.\n"
            "\nResult:\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns the total amount received by the given Vcoin address in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "                           Those recipients will receive less Vcoin than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Vcoin address: ") + name_);
        }
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending Vcoin\n"
            "\nArguments:\n"
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending Vcoin.\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Vcoin address: ") + input.get_str());
            }
...
UniValue z_listunspent(const UniValue& params, bool fHelp)
{
    ...
    std::set<libvcoin::PaymentAddress> zaddrs = {};
    ...
    if (params.size() > 3) {
        ...
        std::set<libvcoin::SproutPaymentAddress> sproutzaddrs = {};
        pwalletMain->GetSproutPaymentAddresses(sproutzaddrs);
        
        // Sapling support
        std::set<libvcoin::SaplingPaymentAddress> saplingzaddrs = {};
    ...
    if (zaddrs.size() > 0) {
        ...
        for (auto & entry : sproutEntries) {
            ...
            bool hasSproutSpendingKey = pwalletMain->HaveSproutSpendingKey(boost::get<libvcoin::SproutPaymentAddress>(entry.address));
        ...
        for (auto & entry : saplingEntries) {
            ...
            pwalletMain->GetSaplingIncomingViewingKey(boost::get<libvcoin::SaplingPaymentAddress>(entry.address), ivk);
...
UniValue zc_raw_receive(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libvcoin::SproutSpendingKey>(&spendingkey) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout spending keys");
    }
    SproutSpendingKey k = boost::get<libvcoin::SproutSpendingKey>(spendingkey);
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    for (const string& name_ : outputs.getKeys()) {
        ...
        if (boost::get<libvcoin::SproutPaymentAddress>(&addrTo) == nullptr) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Only works with Sprout payment addresses");
        }
        CAmount nAmount = AmountFromValue(outputs[name_]);

        vjsout.push_back(JSOutput(boost::get<libvcoin::SproutPaymentAddress>(addrTo), nAmount));
    }
    ...
    {
        auto verifier = libvcoin::ProofVerifier::Strict();
        assert(jsdesc.Verify(*pvcoinParams, verifier, joinSplitPubKey));
    }
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pvcoinParams, joinSplitPubKey);
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nResult:\n"
            "\"vcoinaddress\"    (string) The new shielded address.\n"
            "\nExamples:\n"
...
UniValue z_listaddresses(const UniValue& params, bool fHelp)
{
    ...
    UniValue ret(UniValue::VARR);
    {
        std::set<libvcoin::SproutPaymentAddress> addresses;
    ...
    {
        std::set<libvcoin::SaplingPaymentAddress> addresses;
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libvcoin::SproutPaymentAddress>(&zaddr) != nullptr) {
    ...
    } else if (boost::get<libvcoin::SaplingPaymentAddress>(&zaddr) != nullptr) {
...
UniValue z_sendmany(const UniValue& params, bool fHelp)
{
    ...
    if (!fromTaddr) {
        ...
        fromSapling = boost::get<libvcoin::SaplingPaymentAddress>(&res) != nullptr;
    }
    ...
    for (const UniValue& o : outputs.getValues()) {
        ...
        if (!IsValidDestination(taddr)) {
            ...
            if (IsValidPaymentAddress(res)) {
                ...
                bool toSapling = boost::get<libvcoin::SaplingPaymentAddress>(&res) != nullptr;
...
UniValue z_mergetoaddress(const UniValue& params, bool fHelp)
{
    ...
    if (!fEnableMergeToAddress) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Error: z_mergetoaddress is disabled. Run './vcoin-cli help z_mergetoaddress' for instructions on how to enable this feature.");
    }
    ...
    if (!IsValidDestination(taddr)) {
        ...
        if (IsValidPaymentAddress(decodeAddr)) {
            if (boost::get<libvcoin::SaplingPaymentAddress>(&decodeAddr) != nullptr) {
