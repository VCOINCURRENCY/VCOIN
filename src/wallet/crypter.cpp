                               ...
                               libvcoin::SproutSpendingKey& sk)
{
    ...
    if (vchSecret.size() != libvcoin::SerializedSproutSpendingKeySize)
        return false;
...
static bool DecryptSaplingSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libvcoin::SaplingExtendedSpendingKey& sk)
{
    CKeyingMaterial vchSecret;
...
{
    {
        ...
        {
            const libvcoin::SproutPaymentAddress &address = (*miSprout).first;
        ...
        {
            const libvcoin::SaplingExtendedFullViewingKey &extfvk = (*miSapling).first;
...
bool CCryptoKeyStore::AddSproutSpendingKey(const libvcoin::SproutSpendingKey &sk)
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
bool CCryptoKeyStore::GetSproutSpendingKey(const libvcoin::SproutPaymentAddress &address, libvcoin::SproutSpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
bool CCryptoKeyStore::GetSaplingSpendingKey(const libvcoin::SaplingFullViewingKey &fvk, libvcoin::SaplingExtendedSpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libvcoin::SproutSpendingKey &sk = mSproutSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libvcoin::SproutPaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
