...
{
public:
    libvcoin::SproutPaymentAddress address;
    /**
    ...
    SproutNoteData(libvcoin::SproutPaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    SproutNoteData(libvcoin::SproutPaymentAddress a, uint256 n) :
...
public:
    ...
    SaplingNoteData(libvcoin::SaplingIncomingViewingKey ivk) : ivk {ivk}, witnessHeight {-1}, nullifier() { }
    SaplingNoteData(libvcoin::SaplingIncomingViewingKey ivk, uint256 n) : ivk {ivk}, witnessHeight {-1}, nullifier(n) { }
...
{
    SaplingOutPoint op;
    libvcoin::SaplingPaymentAddress address;
...
public:
    ...
    std::map<libvcoin::SproutPaymentAddress, CKeyMetadata> mapSproutZKeyMetadata;
    std::map<libvcoin::SaplingIncomingViewingKey, CKeyMetadata> mapSaplingZKeyMetadata;
    ...
    libvcoin::SproutPaymentAddress GenerateNewSproutZKey();
    //! Adds spending key to the store, and saves it to disk
    bool AddSproutZKey(const libvcoin::SproutSpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libvcoin::SproutSpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadZKeyMetadata(const libvcoin::SproutPaymentAddress &addr, const CKeyMetadata &meta);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedZKey(const libvcoin::SproutPaymentAddress &addr, const libvcoin::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);
    ...
    bool AddSproutViewingKey(const libvcoin::SproutViewingKey &vk);
    bool RemoveSproutViewingKey(const libvcoin::SproutViewingKey &vk);
    //! Adds a Sprout viewing key to the store, without saving it to disk (used by LoadWallet)
    bool LoadSproutViewingKey(const libvcoin::SproutViewingKey &dest);

    /**
      * Sapling ZKeys
      */
    //! Generates new Sapling key
    libvcoin::SaplingPaymentAddress GenerateNewSaplingZKey();
    //! Adds Sapling spending key to the store, and saves it to disk
    bool AddSaplingZKey(
        const libvcoin::SaplingExtendedSpendingKey &key,
    ...
    bool AddCryptedSaplingSpendingKey(
        ...
        const libvcoin::SaplingPaymentAddress &defaultAddr);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadSaplingZKey(const libvcoin::SaplingExtendedSpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadSaplingZKeyMetadata(const libvcoin::SaplingIncomingViewingKey &ivk, const CKeyMetadata &meta);
    ...
    bool LoadSaplingPaymentAddress(
        ...
        const libvcoin::SaplingIncomingViewingKey &ivk);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedSaplingZKey(const libvcoin::SaplingExtendedFullViewingKey &extfvk,
                                const std::vector<unsigned char> &vchCryptedSecret);
    ...
    std::set<std::pair<libvcoin::PaymentAddress, uint256>> GetNullifiersForAddresses(const std::set<libvcoin::PaymentAddress> & addresses);
    bool IsNoteSproutChange(const std::set<std::pair<libvcoin::PaymentAddress, uint256>> & nullifierSet, const libvcoin::PaymentAddress & address, const JSOutPoint & entry);
    bool IsNoteSaplingChange(const std::set<std::pair<libvcoin::PaymentAddress, uint256>> & nullifierSet, const libvcoin::PaymentAddress & address, const SaplingOutPoint & entry);
    ...
    /** Zelnode Additions */
    /// Get 10000, 25000, 100000, VCC output and keys which can be used for the Zelnode
...
public:
    ...
    bool operator()(const libvcoin::SproutPaymentAddress &zaddr) const;
    bool operator()(const libvcoin::SaplingPaymentAddress &zaddr) const;
    bool operator()(const libvcoin::InvalidEncoding& no) const;
};
...
class GetSpendingKeyForPaymentAddress : public boost::static_visitor<boost::optional<libvcoin::SpendingKey>>
{
private:
    CWallet *m_wallet;
public:
    ...
    boost::optional<libvcoin::SpendingKey> operator()(const libvcoin::SproutPaymentAddress &zaddr) const;
    boost::optional<libvcoin::SpendingKey> operator()(const libvcoin::SaplingPaymentAddress &zaddr) const;
    boost::optional<libvcoin::SpendingKey> operator()(const libvcoin::InvalidEncoding& no) const;
};
...
public:
    ...
    SpendingKeyAddResult operator()(const libvcoin::SproutSpendingKey &sk) const;
    SpendingKeyAddResult operator()(const libvcoin::SaplingExtendedSpendingKey &sk) const;
    SpendingKeyAddResult operator()(const libvcoin::InvalidEncoding& no) const;
};
