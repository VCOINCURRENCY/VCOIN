...
CWalletTx GetValidSproutReceive(const libvcoin::SproutSpendingKey& sk, CAmount value, bool randomInputs, int32_t version = 2) {
    return GetValidSproutReceive(*params, sk, value, randomInputs, version);
}

CWalletTx GetInvalidCommitmentSproutReceive(const libvcoin::SproutSpendingKey& sk, CAmount value, bool randomInputs, int32_t version = 2) {
    return GetInvalidCommitmentSproutReceive(*params, sk, value, randomInputs, version);
}

libvcoin::SproutNote GetSproutNote(const libvcoin::SproutSpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetSproutNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSproutSpend(const libvcoin::SproutSpendingKey& sk,
                        const libvcoin::SproutNote& note, CAmount value) {
...
TEST(WalletTests, SproutNoteDataSerialisation) {
    auto sk = libvcoin::SproutSpendingKey::random();
...
TEST(WalletTests, SetSaplingNoteAddrsInCWalletTx) {
    ...
    libvcoin::SaplingNote note(pk, 50000);
    auto cm = note.cm().get();
...
TEST(WalletTests, CheckSproutNoteCommitmentAgainstNotePlaintext) {
    ...
    ASSERT_THROW(wallet.GetSproutNoteNullifier(
        ...
        address,
        dec,
        hSig, 1), libvcoin::note_decryption_failed);
}
...
TEST(WalletTests, FindMySproutNotes) {
    ...
    auto sk = libvcoin::SproutSpendingKey::random();
    auto sk2 = libvcoin::SproutSpendingKey::random();
...
TEST(WalletTests, GetConflictedSaplingNotes) {
    ...
    auto maybe_pt = libvcoin::SaplingNotePlaintext::decrypt(
...
TEST(WalletTests, SetBestChainIgnoresTxsWithoutShieldedData) {
    ...
    mtxSapling.vShieldedOutput[0].cv = libvcoin::random_uint256();
    ...
    mtxSaplingTransparent.vShieldedOutput[0].cv = libvcoin::random_uint256();
...
TEST(WalletTests, MarkAffectedSaplingTransactionsDirty) {
    ...
    // 0.0005 t-VCC in, 0.0004 z-VCC out, 0.0001 t-VCC fee
    ...
    // 0.0004 z-VCC in, 0.00025 z-VCC out, 0.0001 t-VCC fee, 0.00005 z-VCC change
    auto builder2 = TransactionBuilder(consensusParams, 2, expiryDelta);
