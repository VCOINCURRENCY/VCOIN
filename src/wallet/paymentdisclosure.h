...
#ifndef VCCCASH_PAYMENTDISCLOSURE_H
#define VCCCASH_PAYMENTDISCLOSURE_H
...
struct PaymentDisclosureInfo {
    ...
    uint256 esk;              // vcoin/NoteEncryption.cpp
    uint256 joinSplitPrivKey; // primitives/transaction.h
    // ed25519 - not tied to implementation e.g. libsodium, see ed25519 rfc

    libvcoin::SproutPaymentAddress zaddr;

    PaymentDisclosureInfo() : version(PAYMENT_DISCLOSURE_VERSION_EXPERIMENTAL) {
    }


    PaymentDisclosureInfo(uint8_t v, uint256 esk, uint256 key, libvcoin::SproutPaymentAddress zaddr) : version(v), esk(esk), joinSplitPrivKey(key), zaddr(zaddr) { }
...
struct PaymentDisclosurePayload {
    ...
    libvcoin::SproutPaymentAddress zaddr; // vcoin/Address.hpp
...
#endif // VCCCASH_PAYMENTDISCLOSURE_H
