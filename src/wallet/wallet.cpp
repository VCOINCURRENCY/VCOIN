...
libvcoin::SproutPaymentAddress CWallet::GenerateNewSproutZKey()
{
...
SaplingPaymentAddress CWallet::GenerateNewSaplingZKey()
{
    ...
    auto m = libvcoin::SaplingExtendedSpendingKey::Master(seed);
    ...
    libvcoin::SaplingExtendedSpendingKey xsk;
    do
    {
...
bool CWallet::AddSproutZKey(const libvcoin::SproutSpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapSproutZKeyMetadata
...
bool CWallet::AddCryptedSproutSpendingKey(
    ...
    const libvcoin::ReceivingKey &rk,
...
bool CWallet::AddCryptedSaplingSpendingKey(const libvcoin::SaplingExtendedFullViewingKey &extfvk,
                                           const std::vector<unsigned char> &vchCryptedSecret,
                                           const libvcoin::SaplingPaymentAddress &defaultAddr)
{
...
bool CWallet::LoadCryptedZKey(const libvcoin::SproutPaymentAddress &addr, const libvcoin::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
...
bool CWallet::LoadCryptedSaplingZKey(
    const libvcoin::SaplingExtendedFullViewingKey &extfvk,
    const std::vector<unsigned char> &vchCryptedSecret)
{
     return CCryptoKeyStore::AddCryptedSaplingSpendingKey(extfvk, vchCryptedSecret, extfvk.DefaultAddress());
}

bool CWallet::LoadSaplingZKeyMetadata(const libvcoin::SaplingIncomingViewingKey &ivk, const CKeyMetadata &meta)
{
    AssertLockHeld(cs_wallet); // mapSaplingZKeyMetadata
...
bool CWallet::LoadSaplingZKey(const libvcoin::SaplingExtendedSpendingKey &key)
{
...
bool CWallet::LoadZKey(const libvcoin::SproutSpendingKey &key)
{
    return CCryptoKeyStore::AddSproutSpendingKey(key);
}

bool CWallet::AddSproutViewingKey(const libvcoin::SproutViewingKey &vk)
{
...
bool CWallet::RemoveSproutViewingKey(const libvcoin::SproutViewingKey &vk)
{
...
bool CWallet::LoadSproutViewingKey(const libvcoin::SproutViewingKey &vk)
{
...
std::set<std::pair<libvcoin::PaymentAddress, uint256>> CWallet::GetNullifiersForAddresses(
        const std::set<libvcoin::PaymentAddress> & addresses)
{
    std::set<std::pair<libvcoin::PaymentAddress, uint256>> nullifierSet;
    // Sapling ivk -> list of addrs map
    // (There may be more than one diversified address for a given ivk.)
    std::map<libvcoin::SaplingIncomingViewingKey, std::vector<libvcoin::SaplingPaymentAddress>> ivkMap;
    for (const auto & addr : addresses) {
        auto saplingAddr = boost::get<libvcoin::SaplingPaymentAddress>(&addr);
        if (saplingAddr != nullptr) {
            libvcoin::SaplingIncomingViewingKey ivk;
...
bool CWallet::IsNoteSproutChange(
        const std::set<std::pair<libvcoin::PaymentAddress, uint256>> & nullifierSet,
...
bool CWallet::IsNoteSaplingChange(const std::set<std::pair<libvcoin::PaymentAddress, uint256>> & nullifierSet,
        const libvcoin::PaymentAddress & address,
                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vJoinSplit[i].h_sig(
                            *pvcoinParams, wtxItem.second.joinSplitPubKey);
...
mapSproutNoteData_t CWallet::FindMySproutNotes(const CTransaction &tx) const
{
    ...
    for (size_t i = 0; i < tx.vJoinSplit.size(); i++) {
        auto hSig = tx.vJoinSplit[i].h_sig(*pvcoinParams, tx.joinSplitPubKey);
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (IsLockedCoin((*it).first, i) && nCoinType != ONLY_10000 && nCoinType != ONLY_25000 && nCoinType != ONLY_100000 && nCoinType != ALL_VCCNODE )
                    continue;
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                {
                    LOCK(cs_main);
                    if (NetworkUpgradeActive(chainActive.Height() + 1, Params().GetConsensus(), Consensus::UPGRADE_ACADIA)) {
                        limit = 0;
                    }
                }
                if (limit > 0) {
...
{
    std::set<libvcoin::PaymentAddress> filterAddresses;
    if (address.length() > 0) {
    ...
    bool ignoreUnspendable)
{
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapSproutNoteData) {
            ...
            auto hSig = wtx.vJoinSplit[i].h_sig(*pvcoinParams, wtx.joinSplitPubKey);
            try {
        ...
        for (auto & pair : wtx.mapSaplingNoteData) {
            ...
            if (requireSpendingKey) {
                libvcoin::SaplingIncomingViewingKey ivk;
                libvcoin::SaplingFullViewingKey fvk;
...
//
bool PaymentAddressBelongsToWallet::operator()(const libvcoin::SproutPaymentAddress &zaddr) const
{
    return m_wallet->HaveSproutSpendingKey(zaddr) || m_wallet->HaveSproutViewingKey(zaddr);
}

bool PaymentAddressBelongsToWallet::operator()(const libvcoin::SaplingPaymentAddress &zaddr) const
{
    libvcoin::SaplingIncomingViewingKey ivk;
...
bool PaymentAddressBelongsToWallet::operator()(const libvcoin::InvalidEncoding& no) const
{
    return false;
}

bool HaveSpendingKeyForPaymentAddress::operator()(const libvcoin::SproutPaymentAddress &zaddr) const
{
    return m_wallet->HaveSproutSpendingKey(zaddr);
}

bool HaveSpendingKeyForPaymentAddress::operator()(const libvcoin::SaplingPaymentAddress &zaddr) const
{
    libvcoin::SaplingIncomingViewingKey ivk;
    libvcoin::SaplingFullViewingKey fvk;
...
bool HaveSpendingKeyForPaymentAddress::operator()(const libvcoin::InvalidEncoding& no) const
{
    return false;
}

boost::optional<libvcoin::SpendingKey> GetSpendingKeyForPaymentAddress::operator()(
    const libvcoin::SproutPaymentAddress &zaddr) const
{
    libvcoin::SproutSpendingKey k;
    if (m_wallet->GetSproutSpendingKey(zaddr, k)) {
        return libvcoin::SpendingKey(k);
    } else {
        return boost::none;
    }
}
...
boost::optional<libvcoin::SpendingKey> GetSpendingKeyForPaymentAddress::operator()(
    const libvcoin::SaplingPaymentAddress &zaddr) const
{
    libvcoin::SaplingExtendedSpendingKey extsk;
    if (m_wallet->GetSaplingExtendedSpendingKey(zaddr, extsk)) {
        return libvcoin::SpendingKey(extsk);
    } else {
        return boost::none;
    }
}
...
boost::optional<libvcoin::SpendingKey> GetSpendingKeyForPaymentAddress::operator()(
    const libvcoin::InvalidEncoding& no) const
{
    // Defaults to InvalidEncoding
    return libvcoin::SpendingKey();
}

SpendingKeyAddResult AddSpendingKeyToWallet::operator()(const libvcoin::SproutSpendingKey &sk) const {
...
SpendingKeyAddResult AddSpendingKeyToWallet::operator()(const libvcoin::SaplingExtendedSpendingKey &sk) const {
...
SpendingKeyAddResult AddSpendingKeyToWallet::operator()(const libvcoin::InvalidEncoding& no) const {
