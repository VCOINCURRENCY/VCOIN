...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Vcoin address or script");
    }
...
UniValue rescanblockchain(const UniValue& params, bool fHelp)
{
    ...
    pwalletMain->nTimeFirstKey = 1; // 0 would be considered 'no value'
    pwalletMain->ScanForWalletTransactions(rescanIndex, true);
...
UniValue importwallet_impl(const UniValue& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid Vcoin spending key
        if (fImportZKeys) {
            ...
            if (IsValidSpendingKey(spendingkey)) {
                ...
                // Not a valid spending key, so carry on and see if it's a Vcoin style t-address.
            }
        }
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"folder/filename\"    (string, required) The filename, saved in folder set by vcoind -exportdir option or specified in input. Exportdir option is primary. If no folder is provided, filename will be saved in folder from which the command is executed.\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (fDumpZKeys) {
        std::set<libvcoin::SproutPaymentAddress> sproutAddresses;
        ...
        std::set<libvcoin::SaplingPaymentAddress> saplingAddresses;
...
UniValue z_importviewingkey(const UniValue& params, bool fHelp)
{
    ...
    if (boost::get<libvcoin::SproutViewingKey>(&viewingkey) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Currently, only Sprout viewing keys are supported");
    }
    auto vkey = boost::get<libvcoin::SproutViewingKey>(viewingkey);
...
UniValue z_exportviewingkey(const UniValue& params, bool fHelp)
{
    ...
    // TODO: Add Sapling support. For now, return an error to the user.
    if (boost::get<libvcoin::SproutPaymentAddress>(&address) == nullptr) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Currently, only Sprout zaddrs are supported");
    }
    auto addr = boost::get<libvcoin::SproutPaymentAddress>(address);
