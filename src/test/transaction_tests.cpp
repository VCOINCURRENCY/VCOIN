...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libvcoin's job to ensure the
    ...
    auto k = libvcoin::SproutSpendingKey::random();
    auto addr = k.address();

    libvcoin::SproutNote note(addr.a_pk, 100, uint256(), uint256());
    ...
    std::array<libvcoin::JSInput, ZC_NUM_JS_INPUTS> inputs = {
        libvcoin::JSInput(witness, note, k),
        libvcoin::JSInput() // dummy input of zero value
    };
    std::array<libvcoin::JSOutput, ZC_NUM_JS_OUTPUTS> outputs = {
        libvcoin::JSOutput(addr, 50),
        libvcoin::JSOutput(addr, 50)
    };
    ...
    {
        JSDescription jsdesc(false, *pvcoinParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pvcoinParams, verifier, joinSplitPubKey));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pvcoinParams, verifier, joinSplitPubKey));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(false, *pvcoinParams, joinSplitPubKey, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(false, *pvcoinParams, joinSplitPubKey, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(false, *pvcoinParams, joinSplitPubKey, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pvcoinParams, verifier, joinSplitPubKey));
    }
}
