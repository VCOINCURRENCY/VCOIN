...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_exportwallet)
{
    ...
    libvcoin::SproutPaymentAddress addr = pwalletMain->GenerateNewSproutZKey();
    pwalletMain->GetSproutPaymentAddresses(addrs);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importwallet)
{
    ...
    auto testSpendingKey = libvcoin::SproutSpendingKey::random();
    ...
    std::string format_str = "# Wallet dump created by Vcoin v0.11.2.0.z8-9155cc6-dirty (2016-08-11 11:37:00 -0700)\n"
    ...
    BOOST_ASSERT(boost::get<libvcoin::SproutPaymentAddress>(&address) != nullptr);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_z_importexport)
{
    ...
    BOOST_CHECK(addrs.size()==0);
    std::set<libvcoin::SaplingPaymentAddress> saplingAddrs;
    ...
    BOOST_CHECK(IsValidPaymentAddress(address));
    BOOST_ASSERT(boost::get<libvcoin::SproutPaymentAddress>(&address) != nullptr);
    auto newAddr = boost::get<libvcoin::SproutPaymentAddress>(address);
...
BOOST_AUTO_TEST_CASE(rpc_wallet_encrypted_wallet_sapzkeys)
{
    ...
    // wallet should currently be empty
    std::set<libvcoin::SaplingPaymentAddress> addrs;
...
BOOST_AUTO_TEST_CASE(rpc_z_mergetoaddress_parameters)
{
    ...
    CheckRPCThrows("z_mergetoaddress 1 2",
        "Error: z_mergetoaddress is disabled. Run './vcoin-cli help z_mergetoaddress' for instructions on how to enable this feature.");
