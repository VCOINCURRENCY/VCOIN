...
TEST(keystore_tests, sapling_keys) {
    ...
    for (size_t i = 2; i < 12; i++) {
        ...
        libvcoin::diversifier_t default_d;
        ...
        auto sk = libvcoin::SaplingSpendingKey(skSeed);
        
        // Check that expanded spending key from primitives and from sk are the same
        auto exp_sk_2 = libvcoin::SaplingExpandedSpendingKey(ask, nsk, ovk);
        ...
        auto full_viewing_key_2 = libvcoin::SaplingFullViewingKey(ak, nk, ovk);
        ...
        auto in_viewing_key_2 = libvcoin::SaplingIncomingViewingKey(ivk);
        ...
        auto default_addr_3 = libvcoin::SaplingPaymentAddress(default_d, default_pk_d);
...
TEST(keystore_tests, store_and_retrieve_spending_key) {
    CBasicKeyStore keyStore;
    libvcoin::SproutSpendingKey skOut;

    std::set<libvcoin::SproutPaymentAddress> addrs;
...
TEST(keystore_tests, StoreAndRetrieveViewingKey) {
    CBasicKeyStore keyStore;
    libvcoin::SproutViewingKey vkOut;
    libvcoin::SproutSpendingKey skOut;
    ZCNoteDecryption decOut;
...
TEST(keystore_tests, StoreAndRetrieveSaplingSpendingKey) {
    CBasicKeyStore keyStore;
    libvcoin::SaplingExtendedSpendingKey skOut;
    libvcoin::SaplingFullViewingKey fvkOut;
    libvcoin::SaplingIncomingViewingKey ivkOut;

    std::vector<unsigned char, secure_allocator<unsigned char>> rawSeed(32);
    HDSeed seed(rawSeed);
    auto sk = libvcoin::SaplingExtendedSpendingKey::Master(seed);
...
TEST(keystore_tests, StoreAndRetrieveHDSeedInEncryptedStore) {
    ...
    ASSERT_TRUE(keyStore2.AddSproutSpendingKey(libvcoin::SproutSpendingKey::random()));
...
TEST(keystore_tests, store_and_retrieve_spending_key_in_encrypted_store) {
    ...
    libvcoin::SproutSpendingKey keyOut;
    ZCNoteDecryption decOut;
    std::set<libvcoin::SproutPaymentAddress> addrs;
