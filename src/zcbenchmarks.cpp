...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 joinSplitPubKey;
    auto verifier = libvcoin::ProofVerifier::Strict();
    joinsplit.Verify(*pvcoinParams, verifier, joinSplitPubKey);
    return timer_stop(tv_start);
}
...
double benchmark_large_tx(size_t nInputs)
{
    ...
    auto consensusBranchId = NetworkUpgradeInfo[Consensus::UPGRADE_ACADIA].nBranchId;
    for (size_t i = 0; i < nInputs; i++) {
...
double benchmark_try_decrypt_sprout_notes(size_t nKeys)
{
    ...
    auto sk = libvcoin::SproutSpendingKey::random();
    auto tx = GetValidSproutReceive(*pvcoinParams, sk, 10, true);
...
CWalletTx CreateSproutTxWithNoteData(const libvcoin::SproutSpendingKey& sk) {
    auto wtx = GetValidSproutReceive(*pvcoinParams, sk, 10, true);
    auto note = GetSproutNote(*pvcoinParams, sk, wtx, 0, 1);
...
double benchmark_increment_sprout_note_witnesses(size_t nTxs)
{
    ...
    auto sproutSpendingKey = libvcoin::SproutSpendingKey::random();
...
double benchmark_create_sapling_output()
{
    ...
    libvcoin::SaplingNotePlaintext notePlaintext(note, memo);
