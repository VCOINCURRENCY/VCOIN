...
public:
    ...
    libvcoin::GrothProof zkproof;  //!< A zero-knowledge proof using the spend circuit.
...
public:
    ...
    libvcoin::SaplingEncCiphertext encCiphertext; //!< A ciphertext component for the encrypted output note.
    libvcoin::SaplingOutCiphertext outCiphertext; //!< A ciphertext component for the encrypted output note.
    libvcoin::GrothProof zkproof;   //!< A zero-knowledge proof using the output circuit.
...
public:
    ...
    void operator()(const libvcoin::PHGRProof& proof) const
    {
        if (useGroth) {
    ...
    void operator()(const libvcoin::GrothProof& proof) const
    {
        if (!useGroth) {
...
{
    if (useGroth) {
        libvcoin::GrothProof grothProof;
        ::Unserialize(s, grothProof);
        proof = grothProof;
    } else {
        libvcoin::PHGRProof pghrProof;
...
public:
    ...
    JSDescription(
            ...
            const uint256& rt,
            const std::array<libvcoin::JSInput, ZC_NUM_JS_INPUTS>& inputs,
            const std::array<libvcoin::JSOutput, ZC_NUM_JS_OUTPUTS>& outputs,
            CAmount vpub_old,
            CAmount vpub_new,
    ...
    bool Verify(
        ZCJoinSplit& params,
        libvcoin::ProofVerifier& verifier,
        const uint256& joinSplitPubKey
    ) const;
