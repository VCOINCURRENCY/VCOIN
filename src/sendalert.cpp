...
./vcoind -printtoconsole -sendalert
...
void ThreadSendAlert()
{
    ...
    alert.nID           = 1006;  // use https://github.com/vcoin/vcoin/wiki/specification#assigned-numbers to keep track of alert IDs
    ...
    alert.strComment    = "";
    alert.strStatusBar  = "Your client is outdated. Please update to the most recent version of VCoin (3.3.0 or later).";
    alert.strRPCError   = alert.strStatusBar;

    // Set specific client version/versions here. If setSubVer is empty, no filtering on subver is done:
    // alert.setSubVer.insert(std::string("/MagicBean:0.7.2/"));
    const std::vector<std::string> useragents = {}; //"MagicBean", "BeanStalk", "AppleSeed", "EleosVcoin"};
