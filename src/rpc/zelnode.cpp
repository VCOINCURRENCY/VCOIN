...
UniValue getzelnodeoutputs(const UniValue& params, bool fHelp)
{
    ...
    for (auto& pair : possibleCoins) {
        ...
        obj.push_back(Pair("VCC Amount", pair.second / COIN));
        ret.push_back(obj);
    }
...
UniValue startzelnode(const UniValue& params, bool fHelp)
{
    ...
    if (strCommand == "local") {
        ...
        if (activeZelnode.status != ACTIVE_VCCNODE_STARTED) {
            activeZelnode.status = ACTIVE_VCCNODE_INITIAL; // TODO: consider better way
    ...
    if (strCommand == "all" || strCommand == "missing" || strCommand == "disabled") {
        if ((strCommand == "missing" || strCommand == "disabled") &&
            (zelnodeSync.RequestedZelnodeAssets <= VCCNODE_SYNC_LIST ||
                zelnodeSync.RequestedZelnodeAssets == VCCNODE_SYNC_FAILED)) {
...
UniValue listzelnodes(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
                ...
                "    \"addr\": \"addr\",    (string) Zelnode VCC address\n"
...
UniValue getzelnodestatus (const UniValue& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
                ...
                "  \"addr\": \"xxxx\",        (string) VCC address for zelnode payments\n"
...
UniValue zelnodedebug (const UniValue& params, bool fHelp)
{
    ...
    if (activeZelnode.status != ACTIVE_VCCNODE_INITIAL || !zelnodeSync.IsSynced())
        return activeZelnode.GetStatus();
...
UniValue getzelnodecount (const UniValue& params, bool fHelp)
{
    ...
    zelnodeman.CountNetworks(MIN_PEER_PROTO_VERSION_VCCNODE, ipv4, ipv6, onion);
...
UniValue getzelnodewinners (const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
                ...
                "    \"winner\": {\n"
                "      \"address\": \"xxxx\",    (string) VCC ZN Address\n"
