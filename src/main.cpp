...
{
    bool overwinterActive = NetworkUpgradeActive(nHeight, chainparams.GetConsensus(), Consensus::UPGRADE_ACADIA);
                      ...
                      libvcoin::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vJoinSplit) {
            if (!joinsplit.Verify(*pvcoinParams, verifier, tx.joinSplitPubKey)) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    auto verifier = libvcoin::ProofVerifier::Strict();
    auto disabledVerifier = libvcoin::ProofVerifier::Disabled();
    ...
    const CChainParams& chainparams, CBlockIndex * const pindexPrev)
{
    ...
    // In Vcoin this has been enforced since launch, except that the genesis
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    auto verifier = libvcoin::ProofVerifier::Disabled();
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, const CNode* pfrom, const CBlock* pblock, bool fForceProcessing, CDiskBlockPos* dbp)
{
    ...
    if (zelnodeSync.RequestedZelnodeAssets > VCCNODE_SYNC_LIST) {
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_VCCNODE_WINNER) {
                    if (zelnodePayments.mapZelnodePayeeVotes.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                ...
                if (!pushed && inv.type == MSG_VCCNODE_ANNOUNCE) {
                    if (zelnodeman.mapSeenZelnodeBroadcast.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                ...
                if (!pushed && inv.type == MSG_VCCNODE_PING) {
