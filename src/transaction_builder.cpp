    ...
    std::array<unsigned char, ZC_MEMO_SIZE> memo)
{
    ...
    auto note = libvcoin::SaplingNote(to, value);
    ...
    std::array<unsigned char, ZC_MEMO_SIZE> memo)
{
    ...
    libvcoin::JSOutput jsOutput(to, value);
    jsOutput.memo = memo;
...
void TransactionBuilder::SendChangeTo(libvcoin::SaplingPaymentAddress changeAddr, uint256 ovk)
{
...
void TransactionBuilder::SendChangeTo(libvcoin::SproutPaymentAddress changeAddr)
{
...
TransactionBuilderResult TransactionBuilder::Build()
{
    ...
    if (change > 0) {
        ...
        if (saplingChangeAddr) {
            ...
            libvcoin::SaplingPaymentAddress changeAddr(note.d, note.pk_d);
    ...
    for (auto output : outputs) {
        ...
        libvcoin::SaplingNotePlaintext notePlaintext(output.note, output.memo);
        ...
        libvcoin::SaplingOutgoingPlaintext outPlaintext(output.note.pk_d, encryptor.get_esk());
...
void TransactionBuilder::CreateJSDescriptions()
{
    ...
    std::deque<libvcoin::JSInput> jsInputsDeque;
    for (auto jsInput : jsInputs) {
        jsInputsDeque.push_back(jsInput);
    }
    std::deque<libvcoin::JSOutput> jsOutputsDeque;
    for (auto jsOutput : jsOutputs) {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            ...
            try {
                auto plaintext = libvcoin::SproutNotePlaintext::decrypt(
                    decryptor,
                ...
                vjsin[0] = libvcoin::JSInput(changeWitness.get(), note, changeKey);
        ...
        if (jsOutputsDeque.empty() && jsInputsDeque.empty()) {
            ...
            if (jsInputValue > outAmount) {
                ...
                libvcoin::JSOutput recipientDue(recipient.addr, due);
        ...
        if (jsChange > 0) {
            vjsout[1] = libvcoin::JSOutput(changeAddress, jsChange);
