...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "vcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "vcoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 16325, 26325));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 16324, 26324));
...
)
{
    ...
    if (!(
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Vcoin network parameters in the following directory:\n"
              "%s\n"
    ...
    pvcoinParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-migrationdestaddress")) {
        ...
        libvcoin::PaymentAddress address = DecodePaymentAddress(migrationDestAddress);
        if (boost::get<libvcoin::SaplingPaymentAddress>(&address) == nullptr) {
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Vcoin is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vcoin is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Vcoin is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize Vcoin circuit parameters
    ...
    if (GetBoolArg("-savesproutr1cs", false)) {
        ...
        pvcoinParams->saveR1CS(r1cs_path.string());
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            {
                strErrors << _("Wallet needed to be rewritten: restart Vcoin to complete") << "\n";
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Vcoin was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Vcoin with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Vcoin was not built with wallet support. Set -mineraddress, or rebuild Vcoin with wallet support."));
    }
 #endif // !ENABLE_WALLET
