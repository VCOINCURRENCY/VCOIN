...
{
    return RequestedZelnodeAssets == VCCNODE_SYNC_FINISHED;
}
...
void ZelnodeSync::Reset()
{
    ...
    RequestedZelnodeAssets = VCCNODE_SYNC_INITIAL;
...
void ZelnodeSync::AddedZelnodeList(uint256 hash)
{
    if (zelnodeman.mapSeenZelnodeBroadcast.count(hash)) {
        if (mapSeenSyncZNB[hash] < VCCNODE_SYNC_THRESHOLD) {
...
void ZelnodeSync::AddedZelnodeWinner(uint256 hash)
{
    if (zelnodePayments.mapZelnodePayeeVotes.count(hash)) {
        if (mapSeenSyncZNW[hash] < VCCNODE_SYNC_THRESHOLD) {
...
void ZelnodeSync::GetNextAsset()
{
    switch (RequestedZelnodeAssets) {
        case (VCCNODE_SYNC_FAILED): // should never be used here actually, use Reset() instead
            RequestedZelnodeAssets = VCCNODE_SYNC_INITIAL;
            LogPrintf("ZelnodeSync::GetNextAsset -- Starting %s\n", GetSyncStatus());
            break;
        case(VCCNODE_SYNC_INITIAL):
            ClearFulfilledRequest();
            RequestedZelnodeAssets = VCCNODE_SYNC_SPORKS;
            LogPrintf("ZelnodeSync::GetNextAsset -- Starting %s\n", GetSyncStatus());
            break;
        case (VCCNODE_SYNC_SPORKS):
            RequestedZelnodeAssets = VCCNODE_SYNC_LIST;
            RequestedZelnodeAttempt = 0;
            LogPrintf("ZelnodeSync::GetNextAsset -- Starting %s\n", GetSyncStatus());
            break;
        case (VCCNODE_SYNC_LIST):
            RequestedZelnodeAssets = VCCNODE_SYNC_MNW;
            RequestedZelnodeAttempt = 0;
            LogPrintf("ZelnodeSync::GetNextAsset -- Starting %s\n", GetSyncStatus());
            break;
        case (VCCNODE_SYNC_MNW):
            RequestedZelnodeAssets = VCCNODE_SYNC_FINISHED;
            RequestedZelnodeAttempt = 0;
...
std::string ZelnodeSync::GetSyncStatus()
{
    switch (zelnodeSync.RequestedZelnodeAssets) {
        ...
        case VCCNODE_SYNC_FAILED:
            return _("Synchronization failed");
        case VCCNODE_SYNC_FINISHED:
...
void ZelnodeSync::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (strCommand == "ssc") { //Sync status count
        ...
        if (RequestedZelnodeAssets >= VCCNODE_SYNC_FINISHED) return;
...
void ZelnodeSync::Process()
{
    ...
    if (tick++ % VCCNODE_SYNC_TIMEOUT != 0) return;
    ...
    //try syncing again
    if (RequestedZelnodeAssets == VCCNODE_SYNC_FAILED && lastFailure + (1 * 60) < GetTime()) {
        Reset();
    } else if (RequestedZelnodeAssets == VCCNODE_SYNC_FAILED) {
        return;
    }

    LogPrintf("%s::Process -- Tick %d nCurrentAsset %d\n", __func__, tick, RequestedZelnodeAssets);

    if (RequestedZelnodeAssets == VCCNODE_SYNC_INITIAL) GetNextAsset();

    // sporks synced but blockchain is not, wait until we're almost at a recent block to continue
    if (Params().NetworkID() != CBaseChainParams::REGTEST &&
        !IsBlockchainSynced() && RequestedZelnodeAssets > VCCNODE_SYNC_SPORKS) return;
    ...
    for (CNode* pnode : vNodes) {
        ...
        //set to synced
        if (RequestedZelnodeAssets == VCCNODE_SYNC_SPORKS) {
        ...
        if (pnode->nVersion >= zelnodePayments.GetMinZelnodePaymentsProto()) {
            if (RequestedZelnodeAssets == VCCNODE_SYNC_LIST) {
                LogPrint("zelnode", "%s - lastZelnodeList %lld (GetTime() - VCCNODE_SYNC_TIMEOUT) %lld\n", __func__, lastZelnodeList, GetTime() - VCCNODE_SYNC_TIMEOUT);
                if (lastZelnodeList > 0 && lastZelnodeList < GetTime() - VCCNODE_SYNC_TIMEOUT * 2 && RequestedZelnodeAttempt >= VCCNODE_SYNC_THRESHOLD) { //hasn't received a new item in the last five seconds, so we'll move to the
                    GetNextAsset();
                    return;
                }
                ...
                if (lastZelnodeList == 0 &&
                    (RequestedZelnodeAttempt >= VCCNODE_SYNC_THRESHOLD * 3 || GetTime() - nTimeAssetSyncStarted > VCCNODE_SYNC_TIMEOUT * 10)) {
                    if (IsSporkActive(SPORK_1_VCCNODE_PAYMENT_ENFORCEMENT)) {
                        LogPrintf("%s - ERROR - Syncing zelnode list has failed, failed because %s, will try again\n", __func__, (RequestedZelnodeAttempt >= VCCNODE_SYNC_THRESHOLD * 3) ? "Requested zelnode attempt greater than threshold" : "nTimeAsset Sync timeout");
                        RequestedZelnodeAssets = VCCNODE_SYNC_FAILED;
                        RequestedZelnodeAttempt = 0;
                        lastFailure = GetTime();
                        nCountFailures++;
                    } else {
                ...
                if (RequestedZelnodeAttempt >= VCCNODE_SYNC_THRESHOLD * 3) return;
            ...
            if (RequestedZelnodeAssets == VCCNODE_SYNC_MNW) {
                if (lastZelnodeWinner > 0 && lastZelnodeWinner < GetTime() - VCCNODE_SYNC_TIMEOUT * 2 && RequestedZelnodeAttempt >= VCCNODE_SYNC_THRESHOLD) { //hasn't received a new item in the last five seconds, so we'll move to the
                    GetNextAsset();
                    return;
                }
                ...
                // timeout
                if (lastZelnodeWinner == 0 && (RequestedZelnodeAttempt >= VCCNODE_SYNC_THRESHOLD * 3 || GetTime() - nTimeAssetSyncStarted > VCCNODE_SYNC_TIMEOUT * 10)) {
                    if (IsSporkActive(SPORK_1_VCCNODE_PAYMENT_ENFORCEMENT)) {
