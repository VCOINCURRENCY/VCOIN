...
void Zelnode::Check(bool forceCheck)
{
    ...
    if (!forceCheck && (GetTime() - lastTimeChecked < VCCNODE_CHECK_SECONDS)) return;
    lastTimeChecked = GetTime();


    //once spent, stop doing the checks
    if (activeState == VCCNODE_VIN_SPENT) return;


    if (!IsPingedWithin(VCCNODE_REMOVAL_SECONDS)) {
        activeState = VCCNODE_REMOVE;
        return;
    }

    if (!IsPingedWithin(VCCNODE_EXPIRATION_SECONDS)) {
        activeState = VCCNODE_EXPIRED;
        return;
    }

    if(lastPing.sigTime - sigTime < VCCNODE_MIN_ZNP_SECONDS){
        activeState = VCCNODE_PRE_ENABLED;
        return;
    }
    ...
    if (!unitTest) {
...

        {
            ...
            if (!AcceptableInputs(mempool, state, CTransaction(tx), false, NULL)) {
                activeState = VCCNODE_VIN_SPENT;
                return;
            }
        }
    }

    activeState = VCCNODE_ENABLED; // OK
}
...
bool ZelnodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (!obfuScationSigner.VerifyMessage(pubKeyCollateralAddress, sig, GetStrMessage(), errorMessage))
    {
        ...
        nDos = protocolVersion < MIN_PEER_PROTO_VERSION_VCCNODE ? 0 : 100;
        return error("%s - Got bad Zelnode address signature : %s", __func__, errorMessage);
    }

    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 16325) return false;
    } else if (addr.GetPort() == 16325)
        return false;
    ...
    if (pzn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pzn->IsBroadcastedWithin(VCCNODE_MIN_ZNB_SECONDS)) {
...
bool ZelnodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (GetInputAge(vin) < VCCNODE_MIN_CONFIRMATIONS) {
        LogPrint("zelnode","znb - Input must have at least %d confirmations\n", VCCNODE_MIN_CONFIRMATIONS);
    ...
    // should be at least not earlier than block when 10000, 25000, 100000 VCC tx got VCCNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = uint256();
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pZNIndex = (*mi).second;                                                        // block for 10000, 25000, 100000 VCC tx -> 1 confirmation
        CBlockIndex* pConfIndex = chainActive[pZNIndex->nHeight + VCCNODE_MIN_CONFIRMATIONS - 1]; // block where tx got VCCNODE_MIN_CONFIRMATIONS
        if (pConfIndex->GetBlockTime() > sigTime) {
            LogPrint("zelnode","znb - Bad sigTime %d for Zelnode %s (%i conf block is at %d)\n",
                     sigTime, vin.prevout.hash.ToString(), VCCNODE_MIN_CONFIRMATIONS, pConfIndex->GetBlockTime());
            return false;
        }
    }
...
{
    CInv inv(MSG_VCCNODE_ANNOUNCE, GetHash());
    RelayInv(inv);
}
...
bool ZelnodePing::CheckAndUpdate(int& nDos, bool fRequireEnabled, bool fCheckSigTimeOnly)
{
    ...
    if (pzn != NULL && pzn->protocolVersion >= zelnodePayments.GetMinZelnodePaymentsProto()) {
        ...
        // last ping was more then VCCNODE_MIN_MNP_SECONDS-60 ago comparing to this one
        if (!pzn->IsPingedWithin(VCCNODE_MIN_ZNP_SECONDS - 60, sigTime)) {
...
{
    CInv inv(MSG_VCCNODE_PING, GetHash());
    RelayInv(inv);
}
