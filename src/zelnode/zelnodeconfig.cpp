...
bool ZelnodeConfig::read(std::string& strErr)
{
    ...
    if (!streamConfig.good()) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Zelnode config file\n"
                                    ...
                                    "# Example: zn1 127.0.0.2:16325 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
    ...
    for (std::string line; std::getline(streamConfig, line); linenumber++) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            ...
            strErr = _("Invalid port detected in zelnode.conf") + "\n" +
                     strprintf(_("Line: %d"), linenumber) + "\n\"" + line + "\"" + "\n" +
                     _("(16325 could be used only on mainnet)");
            streamConfig.close();
            return false;
        }
