...
//
// Bootup the Zelnode, look for a 10000 VCC input and register on the network
//
void ActiveZelnode::ManageStatus()
{
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && !zelnodeSync.IsBlockchainSynced()) {
        status = ACTIVE_VCCNODE_SYNC_IN_PROCESS;
        LogPrintf("%s - %s\n", __func__, GetStatus());
        return;
    }

    if (status == ACTIVE_VCCNODE_SYNC_IN_PROCESS) status = ACTIVE_VCCNODE_INITIAL;

    if (status == ACTIVE_VCCNODE_INITIAL) {
        Zelnode* pzn;
        pzn = zelnodeman.Find(pubKeyZelnode);
        if (pzn != NULL) {
            if (!CheckBenchmarks(pzn->tier)) {
                status = ACTIVE_VCCNODE_NOT_CAPABLE;
    ...
    if (status != ACTIVE_VCCNODE_STARTED) {
        // Set defaults
        status = ACTIVE_VCCNODE_NOT_CAPABLE;
        notCapableReason = "";
        ...
        if (GetZelNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            if (GetInputAge(vin) < VCCNODE_MIN_CONFIRMATIONS) {
                status = ACTIVE_VCCNODE_INPUT_TOO_NEW;
...
{
    switch (status) {
        case ACTIVE_VCCNODE_INITIAL:
            return "Node just started, not yet activated";
        case ACTIVE_VCCNODE_SYNC_IN_PROCESS:
            return "Sync in progress. Must wait until sync is complete to start Zelnode";
        case ACTIVE_VCCNODE_INPUT_TOO_NEW:
            return strprintf("Zelnode input must have at least %d confirmations", VCCNODE_MIN_CONFIRMATIONS);
        case ACTIVE_VCCNODE_NOT_CAPABLE:
            return "Not capable zelnode: " + notCapableReason;
        case ACTIVE_VCCNODE_STARTED:
...
bool ActiveZelnode::SendZelnodePing(std::string& errorMessage)
{
    ...
    if (pzn != NULL) {
        if (pzn->IsPingedWithin(VCCNODE_PING_SECONDS, znp.sigTime)) {
...
vector<std::pair<COutput, CAmount>> ActiveZelnode::SelectCoinsZelnode()
{
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == VCCNODE_BASIC_COLLATERAL * COIN) { //exactly
            filteredCoins.push_back(std::make_pair(out, VCCNODE_BASIC_COLLATERAL * COIN));
        }

        else if (out.tx->vout[out.i].nValue == VCCNODE_SUPER_COLLATERAL * COIN) { //exactly
            filteredCoins.push_back(std::make_pair(out, VCCNODE_SUPER_COLLATERAL * COIN));
        }

        else if (out.tx->vout[out.i].nValue == VCCNODE_BAMF_COLLATERAL * COIN) { //exactly
            filteredCoins.push_back(std::make_pair(out, VCCNODE_BAMF_COLLATERAL * COIN));
        }
    }
