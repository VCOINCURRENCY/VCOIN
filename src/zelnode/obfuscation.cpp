...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey, int& nNodeTier)
{
    ...
    if (GetTransaction(vin.prevout.hash, txVin, Params().GetConsensus(), hash, true)) {
        for (CTxOut out : txVin.vout) {
            if (out.nValue == VCCNODE_BASIC_COLLATERAL * COIN) {
                if (out.scriptPubKey == payee2) {
                    nNodeTier = Zelnode::BASIC;
                    return true;
                }
            }
            else if (out.nValue == VCCNODE_SUPER_COLLATERAL * COIN) {
                if (out.scriptPubKey == payee2) {
                    nNodeTier = Zelnode::SUPER;
                    return true;
                }
            }
            else if (out.nValue == VCCNODE_BAMF_COLLATERAL * COIN) {
                if (out.scriptPubKey == payee2) {
...
void ThreadCheckZelnodes()
{
    ...
    RenameThread("vcoin-zelnodescheck");
    ...
    while (true) {
        ...
        if (zelnodeSync.IsBlockchainSynced()) {
            ...
            if (c % VCCNODE_PING_SECONDS == 1) activeZelnode.ManageStatus();
            ...
            if(c % VCCNODES_DUMP_SECONDS == 0) DumpZelnodes();
        }
    }
}
