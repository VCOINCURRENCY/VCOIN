...
#define ZN_WINNER_MINIMUM_AGE 8000    // Age in seconds. This should be > VCCNODE_REMOVAL_SECONDS to avoid misconfigured new nodes in the list.
...
void ZelnodeMan::AskForZN(CNode* pnode, CTxIn& vin)
{
    ...
    int64_t askAgain = GetTime() + VCCNODE_MIN_ZNP_SECONDS;
...
void ZelnodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    // Remove Basic Nodes
    while (basicIt != mapBasicZelnodes.end()) {
        if ((basicIt->second).activeState == Zelnode::VCCNODE_REMOVE ||
            (basicIt->second).activeState == Zelnode::VCCNODE_VIN_SPENT ||
            (forceExpiredRemoval && (basicIt->second).activeState == Zelnode::VCCNODE_EXPIRED) ||
    ...
    // Remove Super Nodes
    while (superIt != mapSuperZelnodes.end()) {
        if ((superIt->second).activeState == Zelnode::VCCNODE_REMOVE ||
            (superIt->second).activeState == Zelnode::VCCNODE_VIN_SPENT ||
            (forceExpiredRemoval && (superIt->second).activeState == Zelnode::VCCNODE_EXPIRED) ||
    ...
    // Remove BAMF Nodes
    while (bamfIt != mapBAMFZelnodes.end()) {
        if ((bamfIt->second).activeState == Zelnode::VCCNODE_REMOVE ||
            (bamfIt->second).activeState == Zelnode::VCCNODE_VIN_SPENT ||
            (forceExpiredRemoval && (bamfIt->second).activeState == Zelnode::VCCNODE_EXPIRED) ||
    ...
    map<uint256, ZelnodeBroadcast>::iterator it3 = mapSeenZelnodeBroadcast.begin();
    while (it3 != mapSeenZelnodeBroadcast.end()) {
        if ((*it3).second.lastPing.sigTime < GetTime() - (VCCNODE_REMOVAL_SECONDS * 2)) {
            mapSeenZelnodeBroadcast.erase(it3++);
    ...
    while (it4 != mapSeenZelnodePing.end()) {
        if ((*it4).second.sigTime < GetTime() - (VCCNODE_REMOVAL_SECONDS * 2)) {
    ...
    for (auto& entry : mapBasicZelnodes) {
        ...
        if (IsSporkActive (SPORK_1_VCCNODE_PAYMENT_ENFORCEMENT)) {
            ...
            if ((nZelnode_Age) < nZelnode_Min_Age) {
                continue; // Skip zelnodes younger than (default) 8000 sec (MUST be > VCCNODE_REMOVAL_SECONDS)
            }
        }
...
void ZelnodeMan::DsegUpdate(CNode* pnode)
{
    ...
    int64_t askAgain = GetTime() + VCCNODES_DSEG_SECONDS;
...
void ZelnodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "znb") { //Zelnode Broadcast
        ...
        for (Zelnode& zelnode : allZelnodes) {
            ...
            if (zelnode.IsEnabled()) {
                ...
                if (vin == CTxIn() || vin == zelnode.vin) {
                    ...
                    pfrom->PushInventory(CInv(MSG_VCCNODE_ANNOUNCE, hash));
                    nInvCount++;
        ...
        if (vin == CTxIn()) {
            pfrom->PushMessage("ssc", VCCNODE_SYNC_LIST, nInvCount);
