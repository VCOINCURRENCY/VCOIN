...
{
    return MIN_PEER_PROTO_VERSION_VCCNODE;
}
...
void Payments::ProcessMessageZelnodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "znget") { //Zelnode Payments Request Sync
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION_VCCNODE) return;
...
bool PaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (pzn->protocolVersion < MIN_PEER_PROTO_VERSION_VCCNODE) {
        strError = strprintf("Zelnode protocol too old %d - req %d", pzn->protocolVersion, MIN_PEER_PROTO_VERSION_VCCNODE);
        LogPrint("zelnode","%s - %s\n", __func__, strError);
        return false;
    }

    int n = zelnodeman.GetZelnodeRank(vinZelnode, nBlockHeight - 100, MIN_PEER_PROTO_VERSION_VCCNODE);
    ...
    if (tier == Zelnode::NONE && IsSporkActive(SPORK_2_VCCNODE_UPGRADE_VOTE_ENFORCEMENT)) {
        strError = strprintf("Zelnode winner tier is %s and %s is active", TierToString(tier), sporkManager.GetSporkNameByID(SPORK_2_VCCNODE_UPGRADE_VOTE_ENFORCEMENT));
        LogPrint("zelnode","%s - %s\n", __func__, strError);
        return false;
    }
...
bool Payments::ProcessBlock(int nBlockHeight)
{
    ...
    int n = zelnodeman.GetZelnodeRank(activeZelnode.vin, nBlockHeight - 100, MIN_PEER_PROTO_VERSION_VCCNODE);
...
{
    CInv inv(MSG_VCCNODE_WINNER, GetHash());
    RelayInv(inv);
}
...
void Payments::Sync(CNode* node, int nCountNeeded)
{
    ...
    while (it != mapZelnodePayeeVotes.end()) {
        ...
        if (winner.nBlockHeight >= nHeight - nCountNeeded && winner.nBlockHeight <= nHeight + 20) {
            node->PushInventory(CInv(MSG_VCCNODE_WINNER, winner.GetHash()));
            nInvCount++;
        }
        ++it;
    }
    node->PushMessage("ssc", VCCNODE_SYNC_MNW, nInvCount);
}
