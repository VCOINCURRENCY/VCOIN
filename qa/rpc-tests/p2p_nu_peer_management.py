...
#
# In this test we connect Sprout, Overwinter, and Sapling mininodes to a Vcoind
# node which will activate Overwinter at block 10 and Sapling at block 15.
#
# We test:
# 1. the mininodes stay connected to Vcoin with Sprout consensus rules
# 2. when Overwinter activates, the Sprout mininodes are dropped
# 3. new Overwinter and Sapling nodes can connect to Vcoin
# 4. new Sprout nodes cannot connect to Vcoin
# 5. when Sapling activates, the Overwinter mininodes are dropped
# 6. new Sapling nodes can connect to Vcoin
# 7. new Sprout and Overwinter nodes cannot connect to Vcoin
#
# This test *does not* verify that prior to each activation, the Vcoind
...
class NUPeerManagementTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Verify mininodes are still connected to vcoind node
        ...
        # Mininodes send ping message to vcoind node.
        pingCounter = 1
        for node in nodes:
        ...
        # Connect a new Overwinter mininode to the vcoind node, which is accepted.
        ...
        # Connect a new Sapling mininode to the vcoind node, which is accepted.
        ...
        # Try to connect a new Sprout mininode to the vcoind node, which is rejected.
        ...
        # Try to connect a new Overwinter mininode to the vcoind node, which is rejected.
