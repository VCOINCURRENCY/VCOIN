#!/usr/bin/env python2
#
# Execute all of the automated tests related to Vcoin.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/vcoin/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/vcoin/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    # Assume that if vcoind is an ELF binary, they all are
    with open(repofile('src/vcoind'), 'rb') as f:
        magic = f.read(4)
        if not magic.startswith(b'\x7fELF'):
            return ret

    ret &= test_rpath_runpath('src/vcoind')
    ret &= test_rpath_runpath('src/vcoin-cli')
    ret &= test_rpath_runpath('src/vcoin-gtest')
    ret &= test_rpath_runpath('src/vcoin-tx')
    ...
    ret &= test_fortify_source('src/vcoind')
    ret &= test_fortify_source('src/vcoin-cli')
    ret &= test_fortify_source('src/vcoin-gtest')
    ret &= test_fortify_source('src/vcoin-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/vcoin-gtest')],
