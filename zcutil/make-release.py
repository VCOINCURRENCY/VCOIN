...
def main(args=sys.argv[1:]):
    """
    Perform the final Vcoin release process up to the git tag.
    """
...
def build():
    ...
    sh_progress([
        ...
        'config.status: creating libvcoinconsensus.pc',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "vcoin-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "vcoin-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './vcoin-make-release.log'
    ...
    logging.info('vcoin make-release.py debug log: %r', logname)
