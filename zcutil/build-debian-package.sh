...
PACKAGE_VERSION=$($SRC_PATH/src/vcoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/vcoind $DEB_BIN
cp $SRC_PATH/src/vcoin-cli $DEB_BIN
...
# Copy manpages
cp $SRC_DOC/man/vcoind.1 $DEB_MAN
cp $SRC_DOC/man/vcoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/zcash-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/vcoind.bash-completion $DEB_CMP/vcoind
cp $SRC_PATH/contrib/vcoin-cli.bash-completion $DEB_CMP/vcoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/vcoind.1
gzip --best -n $DEB_MAN/vcoin-cli.1
gzip --best -n $DEB_MAN/zcash-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/vcoind $DEB_BIN/vcoin-cli
